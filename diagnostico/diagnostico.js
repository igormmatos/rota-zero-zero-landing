(function () {
  ...
  const engine = window.RotaZeroZeroDiagnostic;
  const copy = window.RotaZeroZeroDiagnosticCopy;
  ...
  function progressFor(key) {
    ...
    return {
      label: `Passo ${index + 1} de ${QUESTION_ORDER.length}`,
      value: Math.round(((index + 1) / QUESTION_ORDER.length) * 100),
    };
  }
  ...
  function renderResult() {
    ...
    dynamic.innerHTML = `
      <section class="diagnostic-panel diagnostic-result" data-stage="${escapeHTML(result.stage)}" aria-labelledby="diagnostic-result-title">
        <div class="diagnostic-result-topline">
          <p class="kicker kicker-green">Seu mapa começa aqui</p>
          <span class="diagnostic-version" title="Versão interna do algoritmo">${escapeHTML(result.algorithmVersion)}</span>
        </div>

        <div class="diagnostic-result-hero">
          <div class="diagnostic-stage-node" aria-hidden="true"><span>${escapeHTML(result.stage)}</span></div>
          <div>
            <h1 id="diagnostic-result-title" data-focus-heading>${escapeHTML(text.heading)}</h1>
            <p class="diagnostic-result-body">${escapeHTML(text.body)}</p>
          </div>
        </div>
        ...
        <div class="diagnostic-direction">
          ...
          <div>
            <p class="diagnostic-result-label">Direção agora</p>
        ...
        <div class="diagnostic-boundary">
          <p><strong>Este é um ponto de partida provável, não uma nota.</strong> O diagnóstico organiza sinais para indicar o próximo trecho do mapa. O que fazer, como fazer e o que observar pertence ao Roadmap completo.</p>
        </div>

        <div class="diagnostic-result-actions">
          <a class="button button-dark" href="https://darkslategrey-goat-979048.hostingersite.com/roadmap-completo.html" target="_blank" rel="noopener noreferrer">Conhecer o Roadmap <span aria-hidden="true">→</span></a>
    ...
    announce(`Resultado: ${text.heading}`);
    focusHeading();
  }
